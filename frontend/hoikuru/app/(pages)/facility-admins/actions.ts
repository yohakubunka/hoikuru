...
export const insertFacilityAdminAction = async (
    ...
    const { data: user_data, error: user_error } = await supabaseAdmin.auth.admin.createUser({
        password: password,
        email: email,
...
// 施設情報の取得
export const selectFacilityAdminAction = async ({ facility_admin_id }: { facility_admin_id: number }) => {
    const supabase = await createSupabaseClient();

    const { data, error } = await supabase.from('facility_admins').select().eq('id', facility_admin_id).single();
...
// 施設削除
export const deleteFacilityAdminAction = async (facility_admin_id: number) => {
    const supabase = await createSupabaseClient();
